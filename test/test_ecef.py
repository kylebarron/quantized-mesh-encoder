...
TO_ECEF_TEST_CASES = [
    ([0, 0, 0], [6378137, 0, 0]),
    ([7.43861, 46.951103, 552], [4325328, 564726, 4638459]),
    ([7.81512, 46.30447, 635.0], [4373351, 600250, 4589151]),
    ([7.81471, 46.306686, 635.0], [4373179, 600194, 4589321]), ]
...
def test_to_ecef(llh_positions, exp_positions):
    ...
    assert round(cart_positions[0, 0]) == exp_positions[0]
    assert round(cart_positions[0, 1]) == exp_positions[1]
    assert round(cart_positions[0, 2]) == exp_positions[2]
